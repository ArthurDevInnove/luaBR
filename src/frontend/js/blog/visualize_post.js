  ...
  try {
    ...
    if (!id) {
      window.location.href =
        "http://localhost:5500/src/frontend/html/blog/post_not_found.html";
    }
    ...
    document.querySelector("#author").textContent =
      result.author_name || "Autor desconhecido";
    document.querySelector("#time").textContent =
      `Há ${diffDays} dias, ${diffHours} horas e ${diffMinutes} minutos. ` ||
      "Data desconhecida";
    document.querySelector("#title").textContent =
      result.title || "Título não disponível";
    document.querySelector("#content").textContent =
      result.content || "Conteúdo não disponível";
  } catch (error) {
    console.error(error);
